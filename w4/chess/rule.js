...
function getKnightMoves(currentSquare) {
	...
	available_moves.forEach((move) => {
		...
		if (isSquareEmpty(squareElement)) {
			...
			$(squareElement).addClass('valid-move');
		} else {
			if (
				!$(squareElement)
					.find(':first-child')
...
function getPawnMoves(currentSquare, startSquare) {
	const [row, col] = currentSquare;
	const available_moves = [];
	...
	if (
		valid_moves.some(
			(move) => JSON.stringify(move) === JSON.stringify(startSquare)
		)
	)
		return true;
	return false;
...
function getBishopMoves(currentSquare, startSquare) {
	const [row, col] = currentSquare;
	const available_moves = [];
...
function getRookMoves(currentSquare, startSquare) {
	const [row, col] = currentSquare;
	const available_moves = [];
	...
	available_moves.forEach((move) => {
		...
		if (isSquareEmpty(squareElement)) {
			if (!sameDirection(move, blockSquare)) {
				...
				console.log(move[0], blockSquare[0]);
				if (move[0] < blockSquare[0]) {
					valid_moves.push(move);
					$(squareElement).addClass('valid-move');
				}
			}
...
function getQueenMoves(currentSquare, startSquare) {
